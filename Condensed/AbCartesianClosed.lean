...
Condensed abelian groups form a symmetric monoidal cartesian closed category
...
-/
abbrev CondensedAb := Condensed.{u} AddCommGroupCat

open GrothendieckTopology

instance : HasFiniteProducts (CompHaus.{u}ᵒᵖ ⥤ AddCommGroupCat) := by infer_instance
...
def AsSmall_Functor_equiv :
  (CompHaus.{u}ᵒᵖ ⥤ AddCommGroupCat) ≌ (AsSmall.{u+1} CompHaus.{u}ᵒᵖ ⥤ AddCommGroupCat) :=
Equivalence.congrLeft.{u, u + 1, u + 1, u + 1, u + 1, u + 2}
  AsSmall.equiv.{u, u + 1, u + 1}

noncomputable
example : MonoidalClosed (ModuleCat ℤ) :=
inferInstance
noncomputable
example : MonoidalClosed (AddCommGroupCat) :=
inferInstance

noncomputable
example : MonoidalClosed (AsSmall.{u+1} CompHaus.{u}ᵒᵖ ⥤ ModuleCat ℤ) :=
inferInstance
